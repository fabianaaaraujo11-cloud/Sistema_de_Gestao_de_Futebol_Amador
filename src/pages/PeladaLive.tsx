...
const PeladaLive = () => {
  ...
  const handleRegisterEvent = async (tipo: string, timeId: string, jogadorId: string) => {
    try {
                ...
                {time.jogadores.map(jog => (
                  <div key={jog.id} className="flex justify-between items-center text-sm p-2 hover:bg-gray-50 rounded-lg group">
                    ...
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition">
                      {isOrganizador && (
                        <>
                          <button onClick={() => handleRegisterEvent('gol', time.id, jog.jogador_id)} className="p-1 text-green-600 hover:scale-110" title="Gol"><Trophy className="w-3.5 h-3.5"/></button>
                          ...
                          <button 
                            onClick={() => {
                              const entra = prompt("Digite o ID do jogador que entra da lista de próximas:");
                              if(entra) handleSubstituir(jog.jogador_id, entra);
                            }} 
